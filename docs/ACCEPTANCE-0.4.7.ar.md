...
**يُثبَّت `integration/0.4.7-unified` مرشحًا لتجربة محدودة تحت إشراف محاسب.**

> **تحديث:** حلّ محله للتجربة ثنائية اللغة الوسم `trial-0.4.7-bilingual`؛ انظر [قبول 0.4.7 ثنائي اللغة](ACCEPTANCE-0.4.7-BILINGUAL.ar.md). يبقى `bb70d89` مرجع النسخة العربية وحدها. لا تعديلات مفتوحة بعد هذه النقطة: أي تطوير لاحق يجب أن يستند إلى عطل جديد قابل لإعادة الإنتاج أو احتياج استخدام موثق.
